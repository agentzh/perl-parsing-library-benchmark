...
fanlang:
	@echo === fanlang
	FANLANG_TIMING=0 FANLANG_DEBUG=0 fan -c --no-runtime-check calc.fan
	FANLANG_TIMING=0 FANLANG_DEBUG=0 $(time) resty -e 'require "resty.core" require "jit.opt".start("loopunroll=25", "minstitch=5", "maxtrace=1000", "maxmcode=1000000") -- require "jit.v".on("/dev/stderr")' calc.lua $(datafile)
	@echo

.PHONY: fanlang2
fanlang2:
	@echo === fanlang2
	FANLANG_TIMING=0 FANLANG_DEBUG=0 fan --no-runtime-check -c calc.fan
